<?php
/**
 * Hostinger petition counter API.
 *
 * Public actions:
 * - GET  /api/petition.php?action=get
 * - POST /api/petition.php?action=sign
 * - GET  /api/petition.php?action=flush&token=...
 */
...
const CONFIG = [
    ...
    'db_port' => 3306,
    'counter_id' => 1,
    'buffer_key' => 'cjp_petition_buffer',
    'base_count_key' => 'cjp_petition_base_count',
    'base_count_ttl' => 300,
    'buffer_size' => 1000,
    'dedupe_ttl' => 86400,
...
final class Database
{
    ...
    public function baseCount(Cache $cache): int
    {
        ...
        $stmt = $this->connection()->prepare('SELECT signature_count FROM petition_counter WHERE id = ? LIMIT 1');
    ...
    public function flush(int $increment, Cache $cache): int
    {
        if ($increment > 0) {
            $stmt = $this->connection()->prepare(
                'UPDATE petition_counter
                 SET signature_count = signature_count + ?, last_updated = NOW()
                 WHERE id = ?'
            );
...
final class PetitionController
{
    ...
    {
        try {
            $action = $_GET['action'] ?? ($_SERVER['REQUEST_METHOD'] === 'POST' ? 'sign' : 'get');
            ...
            error_log('Petition counter error: ' . $error->getMessage());
            JsonResponse::error('Petition counter unavailable', 503);
        }
    }
    ...
    private function sign(): void
    {
        ...
        if ($this->isBot()) {
            JsonResponse::ok(['count' => $this->liveCount(), 'status' => 'bot_filtered']);
        }
    ...
    private function dedupeKey(): string
    {
        ...
        return 'cjp_petition_' . substr(hash('sha256', $salt . '|' . $ip), 0, 24);
    }
    ...
    private function isBot(): bool
    {
        ...
        return (bool) preg_match('/bot|crawler|spider|scraper|curl|wget|python|headless|phantom|selenium/i', $userAgent);
    }
